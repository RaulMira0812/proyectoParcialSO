...
void ejecuta_comando(char* cmd, usuario* u) {
	...
	else if (strcmp(cmd_nombre, "TIME")==0) { 
		cmd_argumento = strtok(NULL, " "); 
		cmd_TIME(u,cmd_argumento);
	}		
...
void cmd_MOTD(usuario* u, char* cmd_argumento){
	...
	if (strcasecmp(cmd_argumento, "ESPOLirc")== 0 || cmd_argumento==NULL){
...
void cmd_NAMES(usuario* u, char* cmd_argumento){
	...
	if(cmd_argumento!=NULL){ //muestra los usuarios en el canal especificado
		...
		while (tmp != NULL) {
			...
			sprintf(strbuffer, "[Server]: Usuario '%d' : '%s' \n",cnt,tmp_u->nickname);
			strcat(strbuffertmp,strbuffer);
			 //Envia el mensaje al usuario
			cnt++; 
			tmp = tmp->nodo_siguiente; 
		}mensaje_a_usuario(u, strbuffertmp);
...
void cmd_TIME(usuario *u , char *cmd_argumento){
	if(strcasecmp(cmd_argumento, "ESPOLirc")== 0 || cmd_argumento==NULL){
		...
		char salida[50];
		strftime(salida,50,"%H:%M:%S",hora);
		printf("%s\n",salida);
	
		mensaje_a_usuario(u,salida);
...
void lista_usuarios_server(usuario* u, char* cmd_argumento){ //le muestra al usuario u la lista de todos los usuarios en el server
	if(strcasecmp(cmd_argumento, "ESPOLirc")== 0 || cmd_argumento==NULL){
		...
		while (tmp != NULL) {
			usuario_temporal = (usuario*)(tmp->valor);
			sprintf(strbuffer, ">Usuario %d: %s en el canal %s\n", ++numero_usuario, usuario_temporal->nickname, usuario_temporal->canal_actual->nombre_canal);
			strcat(strbuffertmp,strbuffer);
			tmp = tmp->nodo_siguiente;
		}comando_a_usuario(u, strbuffertmp);
	}else{
		mensaje_a_usuario(u, "[Server]: Nombre del servidor invalido!\n");
		return;
	}	
